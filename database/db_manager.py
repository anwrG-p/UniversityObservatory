...
class DatabaseManager:
    """Thread-safe SQLite connection manager and CRUD repository."""
    ...
    def seed_from_file(self, path: str) -> None:
        """Load a .sql seed file if the opportunities table is empty."""
    ...
    def insert_opportunity(self, data: Dict) -> int:
        sql = """INSERT OR IGNORE INTO opportunities
    ...
    def update_opportunity_category(self, opp_id: int, category: str) -> None:
        self.execute(
            "UPDATE opportunities SET category = ? WHERE id = ?",
            (category, opp_id),
        )

    def update_opportunity_cluster(self, opp_id: int, cluster_id: int) -> None:
        self.execute(
            "UPDATE opportunities SET cluster_id = ? WHERE id = ?",
            (cluster_id, opp_id),
        )
    ...
    def upsert_cluster(self, cluster_id: int, name: str, keywords: str) -> None:
        ...
        if existing:
            self.execute(
                "UPDATE clusters SET name=?, keywords=? WHERE id=?",
                (name, keywords, cluster_id),
            )
        else:
            self.execute(
                "INSERT INTO clusters (id, name, keywords) VALUES (?,?,?)",
                (cluster_id, name, keywords),
            )
    ...
    def insert_recommendation(self, user_id: int, opp_id: int, score: float, reason: str = "") -> None:
        ...
        self.execute(
            "DELETE FROM recommendations WHERE user_id=? AND opportunity_id=?",
            (user_id, opp_id),
        )
        self.execute(
            "INSERT INTO recommendations (user_id, opportunity_id, score, reason) VALUES (?,?,?,?)",
            (user_id, opp_id, score, reason),
        )
    ...
    def insert_notification(self, user_id: int, opp_id: int, message: str) -> None:
        self.execute(
            "INSERT INTO notifications (user_id, opportunity_id, message) VALUES (?,?,?)",
            (user_id, opp_id, message),
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        notif_unread  = self.execute(
            "SELECT COUNT(*) AS n FROM notifications WHERE status='unread'"
        )[0]["n"]
